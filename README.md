...
The layout component allows you to connect multiple generative components (subtractive/additive) as the inputs so that you can evolve multiple building massings with their layout at the same time for small-scale urban design tasks. For connection, you can either directly link the output of "**ComponentID**" on the generative component to the input of "**ComponentIDs**" on the layout component, or you can right click the generative component to copy the component Instance GUID and paste it within a panel, and then connect the panel with the layout component. It is more recommended to use the **second apporoach**. The second approach also allows you to input the desgin generated by one generative component into the layout component multiple times. This can create building layout based on one identical building massing. However, it is not recommended to connect too many generative components as it will significantly increase the search difficulty for the optimization.
...
For generating building layouts, first need to double click the component and to define the user-parameters, including the boundary for layout generation, spacing (optional), and rotation (optional). After finishing the setup, a random building layout will be generated. **Please be aware that the input boundary should have enough space to accomondate all the connected building massings within it**. 
...
If you click the “**Enable Building Spacing**” option, the layout component will try to generate building layout satisfying the specificied spacing distance.

单击“**Enable Building Spacing**”选项后，该组件在生成设计时将尽可能满足输入的间距要求。

### 5.2.2 Rotation 建筑旋转

If you click the “**Enable Rotation**” option, the layout component will generate building massings with varying orientations. However, if you want to generate design with fixed orientation (other than due south direction), you can also enable this option, but input the same value in both the "From" and "To" fields.

单击“**Enable Rotation**”选项后，该组件在生成设计时生成不同朝向的建筑。此外，如果你希望生成特定朝向的建筑，也可以选择该选项，然后在"From"和"To"中输入相同的数值。

### 5.2.3 Conflict Resolution 冲突处理

When there is a conflict between satisfying the boundary contraint and the spacing constraint, you can select the prority of constraint satification. If choose boundary, all the building massings will be kept within the boundary; otherwise, spacing distance will be satisfied.

当在满足边界约束和间距约束存在冲突时，用户可以选择优先满足边界或间距约束。

## 5.3 Optimization 优化

For design optimization, simply connect the layout component with the SSIEA component. The SSIEA component can recognize it and assign required number of parameters for the design generation and optimization. If you want to optimize the building massing at the same time, the SSIEA can also connect the generative components.
